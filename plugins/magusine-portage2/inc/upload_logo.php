...
function traiter_upload_logo($id_groupe=0){
	if (isset($_FILES["logo"])) {
		if ($_FILES["logo"]["error"] == 0) {
			...
			$ok = false;
			if ($chem) {
				$ok = @move_uploaded_file($_FILES['logo']['tmp_name'], $dest);
			}
		...
		} else if ($_FILES["logo"]["error"] == 1 || $_FILES["logo"]["error"] == 2) {
			return "arty:erreur_trop_gros";
		} else if ($_FILES["logo"]["error"] == 3) {
